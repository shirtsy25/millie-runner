...
(function () {
    ...
    Runner.spriteDefinition = {
        LDPI: {
            ...
            CLOUD: { x: 86, y: 2 },
            HORIZON: { x: 2, y: 54 }, //x: 2, y: 54
            MOON: { x: 484, y: 2 },
            PTERODACTYL: { x: 134, y: 2 },
            RESTART: { x: 2, y: 2 },
            TEXT_SPRITE: { x: 655, y: 2 }, // y: 2
            TREX: { x: 848, y: 1 }, //y: 2
            STAR: { x: 645, y: 2 },
            FRISBEE1: {x: 1237,y: 30},
            FRISBEE2: {x:1248, y: 31}
        },
        HDPI: {
    ...
    Runner.prototype = {
         ...
         * Adjusted to play frisbee gif before expanding
         */
        playIntro: function () {
            if (!this.activated && !this.crashed) {
                ...
                introGif.style.height = `${gifHeight}px`;
                introGif.style.bottom = `${gifBottom}px`; // Place GIF on the ground
                
                // position gif next to sprite plus breathing room
                introGif.style.left = `${leftWidth + trex.xPos + Trex.config.WIDTH*2 + 10}px`; 

                //dynamic resizing of gif
                window.addEventListener('resize', () => {
                    const introGif = document.getElementById('intro-gif');
                    if (introGif) {
                        const gifWidth = this.canvas.width;
                        const gifHeight = gifWidth;
                        const groundHeight = this.horizon.getGroundHeight();

                        introGif.style.width = `${gifWidth}px`;
                        introGif.style.height = `${gifHeight}px`;
                        introGif.style.bottom = `${groundHeight}px`;
                    }
                });


                this.containerEl.style.webkitAnimation = 'intro 4.2s ease-out 1 both'; // set intro duration to gif duration. Original .4s
                this.containerEl.style.width = this.dimensions.WIDTH + 'px';

                // Delay the game start to allow the GIF to play
                setTimeout(() => {
                    hideIntro(); 
                }, 4000); //  Adjust to match the GIF duration 1000 = 1s

                //IDK why this is here but it's original
        ...
        gameOver: function () {
            ...
            this.tRex.update(100, Trex.status.CRASHED); //update 100?? sadie
    ...
    /**
     * Frisbee background item.
     * Similar to an obstacle object but without collision boxes.
     * @param {HTMLCanvasElement} canvas Canvas element.
     * @param {Object} spritePos Position of image in sprite.
     * @param {number} containerWidth
     */
    function Frisbee(canvas, spritePos1, spritePos2) {
        this.canvas = canvas;
        this.canvasCtx = this.canvas.getContext('2d');
        ...
        this.frameInterval = 300; // Time in ms to switch frames
    ...
    Frisbee.prototype = {
        /**
         * Initialise the frisbee. Sets the frisbee height.
         */
        init: function () {
            this.draw();
        },
        ...
        draw: function () {
            ...
            var sourceWidth = Frisbee.config.WIDTH;
            var sourceHeight = Frisbee.config.HEIGHT;
            ...
            this.canvasCtx.drawImage(
                Runner.imageSprite, this.currentSprite.x, this.currentSprite.y, 
                sourceWidth, sourceHeight, this.xPos, this.yPos, Frisbee.config.WIDTH, Frisbee.config.HEIGHT 
            );
        ...
        update: function (currentTime) {
            ...
            if (currentTime - this.lastFrameChangeTime > this.frameInterval) {
                this.currentSprite =
                    this.currentSprite === this.spritePos1
                        ? this.spritePos2
                        : this.spritePos1;
            ...
            this.draw(); // Redraw with the new frame
        },
    ...
    function Horizon(canvas, spritePos, dimensions, gapCoefficient) {
        ...
        //Snow
        this.snowEffect = new SnowEffect(canvas, 50);

        //add frisbee to float in top right corner
        if(IS_HIDPI){
            this.frisbee = new Frisbee(this.canvas, {x: 2445,y: 58}, {x:2466, y: 60});
        }else{
            this.frisbee = new Frisbee(this.canvas, {x: 1237,y: 30}, {x: 1237,y: 31});
        }
    ...
    Horizon.prototype = {
        ...
        update: function (deltaTime, currentSpeed, updateObstacles, showNightMode) {
            ...
            if (this.snowEffect) {
                this.snowEffect.update(); // Update snowflake positions
                this.snowEffect.draw();  // Render snowflakes
            }

            if (this.frisbee) {
                this.frisbee.update(performance.now());
            }
        },
        ...
        resize: function (width, height) {
            ...
            if (this.snowEffect) {
                this.snowEffect.flakes = this.snowEffect.generateFlakes(); // Reset snowflakes
            }
        },
         ...
         * @param {number} gifHeight height of Gif
         */
     ...
     */
    class SnowEffect {
        constructor(canvas, numFlakes) {
            this.canvas = canvas;
            this.ctx = canvas.getContext('2d');
            this.numFlakes = numFlakes;
            this.flakes = this.generateFlakes();
        ...
        generateFlakes() {
            ...
            for (let i = 0; i < this.numFlakes; i++) {
                flakes.push({
                    ...
                    size: Math.floor(Math.random() * 3 + 1), // size of snowflakes in pixels
                    speed: Math.random() * .2 + .2 // falling speed
                });
            }
        ...
        /**
         * Update the postion of each flake
         */ 
        update() {
            this.flakes.forEach(flake => {
                flake.y += flake.speed;

                flake.x -= this.horizontalSpeed;
                
                //adjust flakes if they disappear off of the canvas minus a little bit for the horizon top
                if(IS_HIDPI){
                    if (flake.y*2 > this.canvas.height - 30) {
                        flake.y = 0;
                        flake.x = Math.random() * this.canvas.width;
                    }
                }
                else {
                    if (flake.y > this.canvas.height - 15) {
                        flake.y = 0;
                        flake.x = Math.random() * this.canvas.width;
                    }
                }
